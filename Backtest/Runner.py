...
_ORIGINAL = Path(os.environ.get("HERCULES_ORIGINAL_ROOT", _ROOT.parent / "Hercules-3.0")).expanduser()
...
def _build_config_json5(
    ...
    cfg["execution_by_asset"] = {
        asset: {"leverage": float(st.get(asset, {}).get("leverage", portfolio["leverage"]))}
        for asset in _assets
    }
...
def run(
    *,
    ...
    original_package_prefixes = ("Backtest", "Dataframe", "Strategy")
    saved: dict = {
        key: value
