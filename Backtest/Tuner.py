...
def tune(*, candidates: int | None = None, paths: int | None = None) -> dict[str, Any]:
    ...
    search = {**settings["search"], **({"candidates": candidates} if candidates else {}), **({"paths": paths} if paths else {})}
    generator = random.Random(int(search["seed"]))  # noqa: S311 - reproducible parameter search
    best: dict[str, Any] | None = None
    for index in range(int(search["candidates"])):
        ...
        record = {"candidate": index + 1, "passed": passed, "worst_roi_pct": min(path["roi_pct"] for path in metrics), "worst_win_rate": min(path["win_rate"] for path in metrics), "worst_drawdown_pct": min(path["max_drawdown_pct"] for path in metrics), "paths": metrics, "strategy": candidate}
...
def main() -> None:
    ...
    result = tune(candidates=args.candidates, paths=args.paths)
